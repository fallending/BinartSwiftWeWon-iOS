...
struct AvatarHelperTool {
    ...
    public static func calculateSizeWeChatAvatarGroup(_ groupCount: Int, _ containerSize: CGSize, _ distance: CGFloat) -> CGSize {
        ...
        if (groupCount == 1) {
            avatarSize = wholeSize
        } else if groupCount < DCNumberOfGroupAvatarType.Five.description() {
...
extension UIImage {
    ...
    public func cgContextAddArcToPointImage(_ borderRadius: CGFloat, _ borderWidth: CGFloat, _ borderColor: UIColor) -> UIImage {
...
//        path = UIBezierPath(ovalIn: CGRect(x: borderWidth, y: borderWidth, width: avatarImage.size.width, height: avatarImage.size.height))
        ...
        let rect = CGRect(x: borderWidth, y: borderWidth, width: avatarImage.size.width, height: avatarImage.size.height)
        
        path = UIBezierPath.init(roundedRect: rect, byRoundingCorners: UIRectCorner.allCorners, cornerRadii: CGSize(width: borderRadius, height: borderRadius))
